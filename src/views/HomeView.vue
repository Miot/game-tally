...
<template>
  <main class="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-4 px-4 pt-6 pb-10">
    ...
    <button
      v-if="lastRoom"
      type="button"
      class="tap flex items-center justify-between rounded-2xl border border-accent/40 bg-accent/10 px-4 py-3 text-left shadow-sm"
      data-testid="resume-room"
      @click="resumeRoom"
    >
      <span>
        <span class="block text-xs text-ink-2">上次的房间还在</span>
        <span class="readout text-xl tracking-[0.2em] text-accent-deep">{{ lastRoom }}</span>
      </span>
      <span class="font-semibold text-accent-deep">回到房间 →</span>
    </button>
    ...
    <section class="panel overflow-hidden">
      ...
      <div class="p-4">
        ...
        <p class="mt-2 text-[11px] text-ink-3">
          {{ game.designer }} · {{ game.publisher }} · {{ game.year }}
        </p>
      </div>
    </section>
