...
<template>
  <main class="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-4 px-4 pt-3 pb-10">
    ...
    <section class="panel p-4">
      ...
      />
      <p v-if="room.phase === 'joined'" class="mt-2 text-xs text-dust-700">
        返回房间时会把新昵称同步给同桌。
      </p>
    </section>

    <section class="panel p-4">
      <h2 class="mb-1 font-display text-xs tracking-[0.3em] text-dust-500 uppercase">信令策略</h2>
      <p class="mb-3 text-xs text-dust-700">
    ...
    <section class="panel p-4">
      <h2 class="mb-1 font-display text-xs tracking-[0.3em] text-dust-500 uppercase">
        STUN 服务器
      </h2>
      <p class="mb-2 text-xs text-dust-700">每行一个，用于穿透运营商 NAT；默认列表大陆优先。</p>
      <van-field
        v-model="stunText"
        type="textarea"
        rows="4"
    ...
    <section class="panel p-4">
      <div class="mb-1 flex items-center justify-between">
        <h2 class="font-display text-xs tracking-[0.3em] text-dust-500 uppercase">TURN 中转</h2>
        <button type="button" class="tap text-xs text-cyan" @click="addTurn">+ 添加</button>
      </div>
      <p class="mb-2 text-xs text-dust-700">跨运营商蜂窝网络直连失败时才需要。留空则不使用。</p>
      <div
    ...
    <section class="panel p-4">
      <h2 class="mb-2 font-display text-xs tracking-[0.3em] text-dust-500 uppercase">连接诊断</h2>
      <dl class="flex flex-col gap-1 text-xs">
        <div class="flex justify-between gap-2">
          <dt class="text-dust-500">本机对等端 ID</dt>
          <dd class="truncate font-mono text-dust-300">{{ selfId }}</dd>
        </div>
        <div class="flex justify-between gap-2">
          <dt class="text-dust-500">房间状态</dt>
          <dd class="text-dust-300">
            {{
        ...
        <div v-for="relay in room.relays" :key="relay.url" class="flex justify-between gap-2">
          <dt class="truncate text-dust-500">{{ relay.url.replace(/^wss:\/\//, '') }}</dt>
          <dd
            :class="
              relay.state === 'open'
                ? 'text-mint'
        ...
        <p v-if="room.relays.length === 0" class="text-dust-700">进入房间后显示中继连接状态。</p>
      </dl>
      <van-button
