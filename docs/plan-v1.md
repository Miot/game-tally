...
| # | 决策点 | 结论 |
|---|--------|------|
| D1 | 房间同步方案 | 初版用 Trystero（WebRTC）；**2026-09-21 改为公共 MQTT 中继直传**，原因见下方补记。玩家可切换查看其他玩家的分数，但只读 |
| D2 | 计分范围 | 《月球殖民地》只记三项：**幸存者、钱、食物**，用对应的 token 样式展示，配移动端友好的加减按钮 |
| D3 | UI 组件层 | Vant 4 + Tailwind CSS 4（按推荐执行）；**2026-09-20 晚改为白色为主的科技感浅色主题**，令牌见 src/theme/main.css |
| D4 | 加入房间方式 | 4 位房间码 + 分享链接 + 二维码（按推荐执行） |
| D5 | 网络环境 | **需要在中国大陆使用**：默认中继含大陆节点 broker-cn.emqx.io；改为中继直传后不再需要 STUN/TURN |
| D6 | 单机模式 | **不做**「一部手机记全桌」 |
| D7 | PWA | 做（按推荐执行） |
| D8 | 封面与美术 | **2026-09-20 晚改为：首页直接以 BGG 封面图作为游戏入口**（引用 BGG 图片 CDN，不落库；加载失败退回自绘占位）；token 与头像仍为自绘 SVG |
| D9 | 状态模型 | 每人只写自己的状态 + 版本号，入房推送快照，不引入 CRDT（按推荐执行） |

**2026-09-21 同步方案变更**：实测一端 Wi-Fi、一端蜂窝网络时 WebRTC 必然连不上 —— 运营商 NAT 为对称型，穿透失败，而唯一解法 TURN 中转已无可用的免费公共服务（Open Relay 公开凭证全部失效）。改为直接用公共 MQTT 中继转发计分数据：单条约 260 字节，往返约 400 毫秒，不需要 NAT 穿透，任意网络组合可用。传输层接口 RoomTransport 未变，仅替换实现，Trystero 依赖已移除。

对第 2、3 节的修正：2.2 的「垫板人口 + 建筑人口」合并为单一「幸存者」计数器；2.3 第 6 条单机模式删除；快捷行动缩减为与三项计数器相关的采矿 +4 钱、耕作 +4 食物；事件流简化为每位玩家卡片上的「最近变动」。

**2026-09-21 界面方案变更**：2.5 节的「月面基地任务控制台」（深空蓝黑底、青色科技高光、LED 读数）整体作废。用户确认实际使用场景是明亮日光或白光下的桌边，深色底与之冲突；同时产品定位定为通用桌游计分器，绑定单款游戏的美术无法活过第二款游戏。新的视觉世界是「计分纸」——桌游盒里那本预印计分便签：冷白纸、青蓝发丝格线、印刷红印记、两档线宽三档字号、零圆角。信息架构随之改变：房间页上半屏是全桌计分表（一张纸上写着所有人的数），下半屏是只属于自己的写字板，他人视角渲染为 NCR 复写黄联。Vant 组件层已整体移除，控件自写。完整设计系统见 DESIGN.md，方向契约见 .impeccable/surfaces/src-views-roomview-vue.md。
