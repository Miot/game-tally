...
## 2026-09-21 移动端字号上抬：文本三档 11/14/20 → 13/16/24

用户：「整体是需要在移动端上使用的，目前整体的文字偏小，优化下。」

### 依据
排版手册把 **1rem / 16px 定为网页正文的下限**，而当前正文是 14px、标签 11px，都在下限之下。PRODUCT.md 记录的使用场景（桌边、环境常偏暗、抬眼一两秒、手机拿在手里）也要求更大的字。

顺带修掉一个此前没察觉的移动端缺陷：**iOS Safari 在输入框字号小于 16px 时会自动放大整页**。`PadField` 的输入框原本是 14px，昵称与中继地址两处都会触发；提到 16px 后不再发生。

### 改动
- 文本三档 11 / 14 / 20 → **13 / 16 / 24**，数字三档 24 / 40 / 64 不动（已经够大，且上抬会挤爆顶栏）。
- 行高随之放松：label 1.1→1.25、label-cn 1.2→1.3、body 1.5→1.55、lead 1.15→1.2。中文没有 x-height 可借，比拉丁文的常规值再松一档。

### 字号上抬带出的两处溢出，一并修掉
1. **窄屏读数被夹死**：360px 上中间只剩约 76px，64px 的两位数几乎贴住两侧按键。380px 以下主副步进键降一档到 56 / 48px，把宽度让回给那个数；按键仍远在 44px 触控下限之上。
2. **首页规格行第三列放不下**：「幸存者·钱·食物」16px 约 112px，而三等分列在 360px 上只有约 104px。规格行改为「人数｜时长」两列 + 「记这些」整行。

### 验证
390 与 360 两档宽度、首页／房间页／折叠态／展开态／只读黄联／破百禁用全部重拍复核；排版专项扫描与通用检测器均零发现；12 项单测、4 项 e2e、类型检查、lint 通过。DESIGN.md 与 design.json 的字号档位、组件片段里的硬编码字号一并同步，无残留旧值。

## 2026-09-21 首页重排：加入房间升为主角，游戏改两列网格

用户：「虽然目前只有一个游戏，但后续要做成多个游戏还要搜索栏，你重新设计下布局，但不用做搜索栏，让游戏呈两列布局的列表。然后房间号加入的功能要放在顶部显眼的地方，它是主要的功能。」

### 主次为什么该反过来
一桌人里只有一个人开局，其余三四个都是拿着房间号进来的 —— 「加入」的使用频率是「开局」的三四倍。之前那版把「开一局」做成整行黑色大按钮、把加入塞在它下面，主次是反的。用户这个判断有硬依据，不是偏好。

### 结构（自上而下）
1. 品牌头；
2. **加入区（主角）**：「同桌给了你房间号」+ 四格 + 整行 56px 实心石墨「加入这一局」；
3. 上一局入口（仅存在过房间时）；
4. **或者开一局**：两列游戏网格；
5. 页脚说明。

### 房间号四格（新组件 RoomCodeInput）
72px 见方四格，填入字符 40px 石墨粗体。值是一段连续字符串，第 n 格即第 n 个字符，光标永远落在下一个待填格 —— 不允许「第一格空着第三格有字」这种中间空缺；四位码太短，不值得为改中间一位引入一套选区逻辑。点任意一格都把光标收回待填格，所以敲下去的字一定落在该落的地方。支持退格删末位、左右方向键、整串粘贴、自动大写与按字母表过滤。

焦点格边框转印刷红，并为此加了 `.code-box:focus-visible { outline: none }` —— 红边框已是明确的可见焦点，再叠全站那圈石墨外环会读成两层框、像出错。这是全站唯一一处豁免焦点环的位置。

### 游戏网格
两列。卡片＝2px 石墨边的一张纸：1:1 封面（`srcset` 让两列网格取缩略图而非 700px 大图）+ 发丝线分隔的名称区。空着的那一格用 1px 发丝线 + 纸二底写「更多游戏陆续加入」：用更轻的线是为了让「未填的格」轻于「已印上内容的卡」，它不是装饰，是在说明这是个会长的列表。

搜索栏按用户要求不做，也没留占位；「或者开一局」那行标题右侧天然有空间，将来加它不必重排。

### 多游戏只做到版面
网格已按列表铺开，新增游戏只加定义文件即可出现在首页。但 gameId 还没进路由，房间页仍取 defaultGame —— 真加第二款时要把 gameId 带进 URL（房间 ID 本身已由 gameId 与房间码构成）。已记入界面简报的未决项。

### 测试
首页入口从泛指的 `create-room` 改为具体的 `game-{id}`，e2e 相应更新。新增一条四格输入的端到端用例：未填满时提交键禁用、逐格落位、退格删末位后重新禁用、填满提交跳转正确房间。e2e 5 项、单测 12 项、类型检查、lint、检测器零发现全过。360px 窄屏另行复核无溢出。

顺带修掉上一轮字号上抬时漏掉的一处不一致：`design.json` 的散文里仍写着「11px 角标／20px 资源名／14px 粗体」，与已改的 13 / 16 / 24 矛盾。当时只替换了数值令牌，没替换文字描述。由文档者在本轮核对中发现并报告，现已改正（其余 20px 是阴影模糊值与 CSS 尺寸，不是字号，未动）。

## 2026-09-21 房间页收紧：主界面回到一屏之内

用户：「房间页面的布局和字体有点大，显得很紧凑，优化下。」

### 诊断比「紧凑」更严重
量出来的结果：页面高 868px、视口 844px，**食物行的行动键已经被挤出视口**，主界面需要滚动才能看全。

密度还是不均的：钱／食物行内容 220px、分到 224px，几乎零空白；幸存者行没有行动键，内容只有 152px，空出 72px。一边挤死一边空着。

### 回收了一处重复结构
注解（起始 N + 铅笔痕）当初移到读数正下方，是为了躲开**当时还在标题行**的「采矿 +4」。后来行动键搬到了读数下方，标题行右端就空了 —— 注解搬回去白省一整行，不牺牲任何信息。

顺带：行距 gap-3 → gap-2、行动键 56 → 48px（与底部跨资源行动条同档）、资源名块 py-1 → py-0.5、写字板表头 py-2 → py-1.5。

字号档位一个没动 —— 上一轮刚按移动端 16px 下限调过，问题不在字号本身，在结构冗余。

### 立了一条规则
DESIGN.md 新增 **The One Screen Rule**：房间页在目标视口内必须一屏装下，任何一行的内容都不得逼近它分到的高度，挤满即是溢出的前兆；需要腾地方时**先回收重复结构，再动字号与按键**。这次正是按这个顺序解决的。

### 验证
在截图脚本里加了高度量测，不再靠目测：390×844 与 360×780 两档均为「文档高度 = 视口高度」，正好装下。折叠态／展开态／窄屏三种情形的三个资源行都完整可见。e2e 5 项、单测 12 项、类型检查、lint、检测器零发现。

## 2026-09-21 写字板表头收小

用户：「『我的一栏』这一列连同右边的撤销按钮，做小一点。」

撤销是低频的补救操作，却占着「图标 + 文字 + 2px 边框」的一整块（约 90×44）。收成 44×44 的纯图标方块，宽度减半；触控面仍是 44px 未破下限，可访问名由 `aria-label="撤销"` 承担，e2e 的 `getByRole('button', { name: '撤销' })` 照常通过。表头纵向内边距 py-1.5 → py-1。

省下的高度落到三个资源行上，房间页仍是 390×844 与 360×780 两档「文档高度 = 视口高度」。
